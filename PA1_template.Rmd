...
---
title: "Reproducible Research course repdata-011 - Project 1"
...
Median is marked yellow and the mean value is green.

3. Let us calculate and report the mean and median of the total number of steps taken per day.
```{r}
mean(steps_hist)
median(steps_hist)
```
...
m<-m + scale_x_continuous(breaks=seq(0,1440,60),labels=seq(0,24,1))+geom_line(colour="blue",size=1)
print(m)
```
As we can see, patterns are different.
